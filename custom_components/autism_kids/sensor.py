...
def _kid_home(hass: HomeAssistant, entry: ConfigEntry, coordinator: AutismKidsCoordinator) -> str:
    ...
    person_one_name = entry.data.get(CONF_PERSON_ONE_NAME, "Person 1")
    person_two_name = entry.data.get(CONF_PERSON_TWO_NAME, "Person 2")
    if person_one and _state(hass, person_one) == "home":
        names.append(person_one_name)
    if person_two and _state(hass, person_two) == "home":
        names.append(person_two_name)
    if not names:
...
def _school_tomorrow(hass: HomeAssistant, entry: ConfigEntry, coordinator: AutismKidsCoordinator) -> str:
    ...
    return "School tomorrow" if state == "on" else "No school tomorrow"
